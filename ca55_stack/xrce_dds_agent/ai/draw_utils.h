/**
 * @file draw_utils.h
 * @brief Shared drawing utilities for BGR frame annotation (no OpenCV dependency)
 *
 * Provides lightweight pixel-level drawing functions for bounding boxes,
 * text labels, and detection overlays on raw BGR888 buffers.
 *
 * Extracted from ai_camera_test.cpp for reuse across ai_camera_test and
 * image_streamer modules.
 */
                       ...
                       char ch, uint8_t r, uint8_t g, uint8_t b)
{
    ...
    else return; // skip spaces, punctuation
...
/* ── Draw detections in model-space coordinates (640x640) ──────── */

inline void draw_detections_on_bgr(uint8_t* bgr, int w, int h,
                                     const std::vector<Detection>& dets)
{
    const auto* colors = class_colors();
    for (const auto& d : dets) {
        int x1 = (int)d.bbox.x1, y1 = (int)d.bbox.y1;
        int x2 = (int)d.bbox.x2, y2 = (int)d.bbox.y2;
        int ci = d.class_id % 12;
        uint8_t r = colors[ci][0], g = colors[ci][1], b = colors[ci][2];

        draw_rect(bgr, w, h, x1, y1, x2, y2, r, g, b, 2);
        ...
        for (int ly = y1 - 9; ly < y1 - 1; ++ly)
            for (int lx = x1; lx < x1 + lw && lx < w; ++lx)
                draw_pixel(bgr, w, h, lx, ly, r, g, b);

        draw_text(bgr, w, h, x1 + 1, y1 - 8, label, 255, 255, 255);
    }
}
 ...
 * The camera frame is cam_w x cam_h (e.g. 640x480).
 * Letterbox preprocessing pads top/bottom symmetrically.
 *
 * Mapping: x_cam = x_model, y_cam = y_model - pad_top
 * where pad_top = (model_size - cam_h) / 2
 */
                                     ...
                                     int model_size)
{
    ...
    for (const auto& d : dets) {
        ...
        if (x1 >= x2 || y1 >= y2) continue; // fully outside
        ...
        int lw = (int)strlen(label) * 6 + 2;
        int label_y = y1 - 9;
        if (label_y < 0) label_y = y2 + 2; // move below if too close to top

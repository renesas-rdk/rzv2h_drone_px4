...
*                 not 0 otherwise
******************************************/
/* LoadFileToMemDynamic commented out to avoid missing header dependency */
/*
...
uint8_t PreRuntime::Load(const std::string pre_dir, uint32_t start_addr, uint8_t mode)
{
    ...
    for (i = 1;i<NUM_OBJ_FILE; i++)
    {
        ...
        else
        {
            ...
            ret = ReadFileData(tmp_data, drpai_file_path[i], size);
            if ( PRE_SUCCESS < ret )
            {
                std::cerr << "[ERROR] Failed to read file "<< drpai_file_path[i] << std::endl;
                return PRE_ERROR;
            }
            ret = LoadDataToMem(tmp_data, addr+drpai_obj_info.data_inout.start_address, size);
        }
        /*Error Check*/
        if ( PRE_SUCCESS < ret )
        {

...
{
    int (* send_offchannel_raw)(struct rpmsg_device * rdev, uint32_t src, uint32_t dst, const void * data, int size,
                                int wait);
};
...
static inline int rpmsg_send (struct rpmsg_endpoint * ept, const void * data, int len)
{
    ...
    return rpmsg_send_offchannel_raw(ept, ept->addr, ept->dest_addr, data, len, true);
}
...
static inline int rpmsg_trysend (struct rpmsg_endpoint * ept, const void * data, int len)
{
    ...
    return rpmsg_send_offchannel_raw(ept, ept->addr, ept->dest_addr, data, len, false);
}

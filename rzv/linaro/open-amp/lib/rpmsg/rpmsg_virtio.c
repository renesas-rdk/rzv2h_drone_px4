 ...
 */
static void * rpmsg_virtio_get_tx_buffer (struct rpmsg_virtio_device * rvdev, unsigned long * len, unsigned short * idx)
{
    unsigned int role = rpmsg_virtio_get_role(rvdev);
    void       * data = NULL;
    ...
    if (role == RPMSG_MASTER)
    {
        ...
        {
            data = rpmsg_virtio_shm_pool_get_buffer(rvdev->shpool, RPMSG_BUFFER_SIZE);
 ...
 *
 */
static void * rpmsg_virtio_get_rx_buffer (struct rpmsg_virtio_device * rvdev, unsigned long * len, unsigned short * idx)
{
    unsigned int role = rpmsg_virtio_get_role(rvdev);
    void       * data = NULL;
    ...
    {
        data =
            virtqueue_get_available_buffer(rvdev->rvq, idx, (uint32_t *) len);
    }
#endif                                 /*!VIRTIO_MASTER_ONLY*/
...
{
    uint8_t status;
    /* Wait for master indefinitely, but yield/sleep to avoid busy spinning. */
    while (1)
    {
                                             ...
                                             int                   wait)
{
    ...
    io     = rvdev->shbuf_io;
    status = metal_io_block_write(io, metal_io_virt_to_offset(io, buffer), &rp_hdr, sizeof(rp_hdr));
    RPMSG_ASSERT(status == sizeof(rp_hdr), "failed to write header\n");

    status = metal_io_block_write(io, metal_io_virt_to_offset(io, RPMSG_LOCATE_DATA(buffer)), data, size);
...
static void rpmsg_virtio_rx_callback (struct virtqueue * vq)
{
    ...
    rp_hdr = (struct rpmsg_hdr *) rpmsg_virtio_get_rx_buffer(rvdev, &len, &idx);
    ...
    while (rp_hdr)
    {
        ...
        status = ept->cb(ept, (void *) RPMSG_LOCATE_DATA(rp_hdr), rp_hdr->len, ept->addr, ept->priv);

        RPMSG_ASSERT(status == RPMSG_SUCCESS, "unexpected callback status\n");
        metal_mutex_acquire(&rdev->lock);
...
static int rpmsg_virtio_ns_callback (struct rpmsg_endpoint * ept, void * data, size_t len, uint32_t src, void * priv)
{
    ...
    metal_io_block_read(io, metal_io_virt_to_offset(io, ns_msg->name), &name, sizeof(name));
    dest = ns_msg->addr;
                     ...
                     struct rpmsg_virtio_shm_pool * shpool)
{
    ...
    if (role == RPMSG_REMOTE)
    {
        ...
        int ready = rpmsg_virtio_wait_remote_ready(rvdev);
        if (ready != RPMSG_SUCCESS) {
            return ready;
        }
    }
#endif                                 /*!VIRTIO_MASTER_ONLY*/
    ...
    status = rpmsg_virtio_create_virtqueues(rvdev, 0, RPMSG_NUM_VRINGS, vq_names, callback);
    ...
    if (role == RPMSG_MASTER)
    {
        ...
        for (idx = 0; idx < rvdev->rvq->vq_nentries; idx++)
        {
            ...
            buffer = rpmsg_virtio_shm_pool_get_buffer(shpool, RPMSG_BUFFER_SIZE);
            ...
            metal_io_block_set(shm_io, metal_io_virt_to_offset(shm_io, buffer), 0x00, RPMSG_BUFFER_SIZE);
            status =
                virtqueue_add_buffer(rvdev->rvq, &vqbuf, 0, 1, buffer);
    ...
    {
        rpmsg_init_ept(&rdev->ns_ept, "NS", RPMSG_NS_EPT_ADDR, RPMSG_NS_EPT_ADDR, rpmsg_virtio_ns_callback, NULL);

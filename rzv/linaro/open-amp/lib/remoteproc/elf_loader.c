                                           ...
                                           size_t            * nmsize)
{
    ...
    while (p_type != PT_LOAD)
    {
        ...
        elf_parse_segment(elf_info, phdr, &p_type, noffset, da, NULL, nfsize, nmsize);
        *nseg = *nseg + 1;
    }
                     ...
                     size_t     * nlen)
{
    ...
    if ((*load_state & ELF_STATE_WAIT_FOR_SHSTRTAB) != 0)
    {
        ...
        elf_parse_section(*img_info, shdr, NULL, NULL, NULL, &shstrtab_offset, &shstrtab_size, NULL, NULL, NULL, NULL);
        ...
        *shstrtab        = metal_allocate_memory(shstrtab_size);
        ...
        memcpy(*shstrtab, (const void *) ((const char *) img_data + shstrtab_offset), shstrtab_size);
        *load_state = (*load_state & (~ELF_STATE_MASK)) |
                      ELF_STATE_HDRS_COMPLETE;
        *nlen = 0;
              ...
              size_t            * nmemsize)
{
    ...
    if ((last_load_state & RPROC_LOADER_MASK) == RPROC_LOADER_NOT_READY)
    {
        ...
        last_load_state = elf_load_header(img_data, offset, len, img_info, last_load_state, noffset, nlen);
...
int elf_locate_rsc_table (void * elf_info, metal_phys_addr_t * da, size_t * offset, size_t * size)
{
    ...
    elf_parse_section(elf_info, shdr, NULL, NULL, da, offset, size, NULL, NULL, NULL, NULL);
